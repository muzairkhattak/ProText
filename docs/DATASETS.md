...
### Acknowledgement: This readme file for installing datasets has been borrowed from [PromptSRC's](https://github.com/muzairkhattak/PromptSRC) official repository with partial modifications.

**NOTE:** ProText does not use image samples during training. The following steps to configure datasets are mainly for evaluation purposes. Additionally, they can be used to train image-based methods such as PromptSRC and MaPLe.
